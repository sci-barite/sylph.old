...
chrome.runtime.onMessage.addListener((request, sender) => {
    ...
    if (request.name == 'Sylph') {
        switch (request.site.substring(12, 18)) {
            case "linked":
                SiftLinked(request.position);
                break;
            case "ni.co/":
                SiftDjinni(request.position);
                break;
            case "upwork":
                SiftUpwork(request.site);
                break; // The function should check if it's a profile or proposal page!
            default:
                alert(request.site.substring(12, 18) + ": Can't read website name!");
                return;
        }
        ...
        XSnd.onreadystatechange = () => {
            if (XSnd.readyState === XMLHttpRequest.DONE) {
                if (XSnd.status === 0 || (XSnd.status >= 200 && XSnd.status < 400))
                    SylphBack(XSnd.response, XSnd.status);
                else {
                    alert("⛔ ERROR!\nStatus: " + XSnd.status + "\nSylph didn't find her way home!");
                    chrome.runtime.sendMessage({ SpellSuccessful: false }); // Update icon to show something's wrong...
                }
        ...
        'https://script.google.com/macros/s/AKfycbyZb43hadRmFpjDg1ynHnY31z6yIPT0tzaSbNMBNcBB76dfPWCssOXFTfwXRVGGzrZ0/exec?' +
            ...
            + '&skills=' + SKILLS + '&eng=' + ENGLISH + '&rate=' + RATE + '&loc=' + LOCATION + '&url=' + LINK + '&more=' + MORE, true);
        XSnd.send();
    }
});
